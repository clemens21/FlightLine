...
function renderPilotHiringMarket(saveId, tabId, source, options = {}) {
    ...
    const columnGroup = `<colgroup><col style="width:260px" /><col style="width:220px" /><col style="width:130px" /><col style="width:150px" /><col style="width:150px" /><col style="width:150px" /><col style="width:150px" /><col style="width:170px" /><col style="width:210px" /></colgroup>`;
...
function renderBaseAirportField(defaultAirportId) {
    const normalizedAirportId = typeof defaultAirportId === "string" ? defaultAirportId.trim().toUpperCase() : "";
    return `<label class="staffing-hire-base-field"><span class="eyebrow">Base airport</span><input type="text" name="baseAirportId" value="${escapeHtml(normalizedAirportId)}" placeholder="Airport code" autocapitalize="characters" autocomplete="off" spellcheck="false" inputmode="text" data-staffing-base-airport-input /></label>`;
}
...
function renderHireCandidateDetail(saveId, tabId, candidate, context) {
    ...
    const comparisonCards = [directOffer, contractOffer]
        ...
        .join("");
    const pilotSnapshotRows = renderStaffingFactRow("Current location", currentAirport ? `${currentAirport.code} | ${currentAirport.primaryLabel}` : "Unassigned location")
