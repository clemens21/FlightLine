...
import {
  renderContractsAircraftSharedTableStyles,
  renderSharedTableHeaderStyles,
} from "./save-shell-table-styles.js";
...
export function renderIncrementalSavePage(saveId: string, activeTab: SavePageTab, saveShellClientAssetPath: string): string {
    const configJson = JSON.stringify({ saveId, initialTab: activeTab })
        ...
        .replaceAll("<", "\\u003c")
        .replaceAll(">", "\\u003e");
    const sharedTableHeaderStyles = renderSharedTableHeaderStyles();
    const contractsAircraftSharedTableStyles = renderContractsAircraftSharedTableStyles();
    return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .contracts-planning-panel > .panel-head,
    .planner-table-panel > .panel-head,
    .planner-chain-panel > .panel-head {
    ...
    .planner-setup-line {
      display: grid;
      grid-template-columns: minmax(0, 1.05fr) minmax(0, .9fr) minmax(220px, .9fr) auto;
      gap: 12px;
      align-items: end;
      padding-bottom: 6px;
      border-bottom: 1px solid var(--line);
    ...
    ${contractsAircraftSharedTableStyles}
